...
const Features = memo(() => {
  return (
    <section 
      id="features" 
      className="py-32 bg-dark-900 relative"
    ...
    >
      <div className="container mx-auto px-6">
        <div className="mb-20">
          ...
          <h2 
            id="features-heading"
            className="font-display text-5xl md:text-7xl text-white uppercase"
          >
...
const FeatureCard = memo(({ feature, index }: FeatureCardProps) => {
  ...
  return (
    ...
    >
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-lime-400 to-transparent transform -translate-x-full group-hover:translate-x-0 transition-transform duration-500" />
      
      <div 
        className="text-lime-400 mb-6 group-hover:scale-110 transition-transform duration-300 origin-left"
        aria-hidden="true"
      >
        {feature.icon}
      </div>
      ...
      <div 
        className="absolute bottom-4 right-4 text-neutral-800 font-display text-6xl opacity-20 select-none group-hover:opacity-40 transition-opacity pointer-events-none"
        aria-hidden="true"
      >
        0{index + 1}
      </div>
